...
# -----------------------------------------------------------------------------
import os, sys, math, time, struct, socket
...
def rxHandleDevices( ip, stack, address, ports, memory, files, inputs ):
  ...
  if ports[5] == -1:  # memory size - zero based index
    ...
    ports[5] = 0
  elif ports[15] == 1: # client connect
    port = stack.pop()
    addrPointer = stack.pop()
    addr = ""
    ndx = 0
    global socketIndex
    while memory[addrPointer + ndx] != 0:
        addr += chr(memory[addrPointer + ndx])
        ndx += 1
    print "Opened Address: %s Port: %i" % (addr,port)
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    totalSockets.append(s)
    totalSockets[socketIndex].connect((addr,port))
    stack.append(socketIndex)
    socketIndex += 1
    ports[15] = 0
  elif ports[15] == 2: # client close
    socketNdx = stack.pop()
    totalSockets[socketNdx].close()
    print "Closed Socket"
    stack.append(1)
    ports[15] = 0
  elif ports[15] == 3: # client read
    storBuffer = stack.pop()
    bufsize = stack.pop()
    socketNdx = stack.pop()
    rcvd = totalSockets[socketNdx].recv(bufsize)
    print "Read: %i bytes" % len(rcvd)
    for x in range(len(rcvd)):
        memory[storBuffer + x] = struct.unpack(">B",rcvd[x])[0]
    stack.append(len(rcvd))
    ports[15] = 0    
  elif ports[15] == 4: # client write
    sendBuffer = stack.pop()
    bufsize = stack.pop()
    socketNdx = stack.pop()
    bytes = ""
    for i in range(bufsize):
        bytes += struct.pack(">b", memory[sendBuffer + i])
    print "Wrote: %i" % len(bytes)
    num = totalSockets[socketNdx].send(bytes)
    stack.append(num)
    ports[15] = 0
  return ip
